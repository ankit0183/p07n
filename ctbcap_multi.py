...
class DownloadQueueConfig:
    ...
    fetch_retry_delay: float = 3.0
    min_free_space_mib: int = 1024
    adaptive_concurrency: bool = True
    max_concurrent_downloads_high: int = 25
    max_concurrent_downloads_low: int = 5
...
class GlobalConfig:
    ...
    adaptive_check_interval: bool = True
    min_check_interval: int = 10
    max_check_interval: int = 300
    priority_boost_online: bool = True
    auto_discover_enabled: bool = True
    auto_discover_interval: int = 600
    auto_discover_platform: str = "stripchat"
    auto_discover_limit: int = 500
    auto_add_new_models: bool = False
...
class Config:
    ...
    def from_yaml(cls, path: str) -> "Config":
        ...
        global_config = GlobalConfig(
            ...
            check_interval=global_data.get('check_interval', 30),
            retry_interval=global_data.get('retry_interval', 30),
            ...
            adaptive_check_interval=global_data.get('adaptive_check_interval', True),
            min_check_interval=global_data.get('min_check_interval', 10),
            max_check_interval=global_data.get('max_check_interval', 300),
            priority_boost_online=global_data.get('priority_boost_online', True),
            auto_discover_enabled=global_data.get('auto_discover_enabled', True),
            auto_discover_interval=global_data.get('auto_discover_interval', 600),
            auto_discover_platform=global_data.get('auto_discover_platform', 'stripchat'),
            auto_discover_limit=global_data.get('auto_discover_limit', 500),
            auto_add_new_models=global_data.get('auto_add_new_models', False),
            web_ui_enabled=global_data.get('web_ui_enabled', False),
            web_ui_port=global_data.get('web_ui_port', 8081),
            bandwidth_limit_mbps=global_data.get('bandwidth_limit_mbps', 0),
            cpu_limit_percent=global_data.get('cpu_limit_percent', 80),
        )
        models = []
...
class ConfigManager:
    ...
    def add_model(self, name: str, platform: str = None, check_interval: int = None,
                  ...
                  save_path: str = None, log_path: str = None, priority: int = 0) -> bool:
        self._ensure_global()
        ...
        if check_interval is None:
            check_interval = self._data['global'].get('check_interval', 30)
        if retry_interval is None:
            retry_interval = self._data['global'].get('retry_interval', 30)
        if cut_time is None:
    ...
    def add_models_bulk(self, models_list: List[Dict[str, Any]]) -> int:
        ...
        for m in models_list:
            ...
            check_interval = m.get('check_interval', self._data['global'].get('check_interval', 30))
            retry_interval = m.get('retry_interval', self._data['global'].get('retry_interval', 30))
...
class DownloadQueue:
    def __init__(self, config: DownloadQueueConfig):
        ...
        self._fetch_semaphore = asyncio.Semaphore(config.max_concurrent_fetches)
        self.offline_grace_tasks: Dict[str, asyncio.Task] = {}
        self.logger = logging.getLogger("download_queue")
        self._current_download_limit = config.max_concurrent_downloads
    ...
    async def start_adaptive_concurrency(self, recorder: 'Recorder', disk_monitor: 'DiskSpaceMonitor'):
        """Start adaptive concurrency adjustment based on system resources."""
        if not self.config.adaptive_concurrency:
            return
        self._adaptive_task = asyncio.create_task(self._adaptive_concurrency_loop(recorder, disk_monitor))

    async def stop_adaptive_concurrency(self):
    ...
    async def _adaptive_concurrency_loop(self, recorder: 'Recorder', disk_monitor: 'DiskSpaceMonitor'):
        while True:
            try:
                await asyncio.sleep(30)
                await self._adjust_concurrency(recorder, disk_monitor)
            except asyncio.CancelledError:
                break
            except Exception as e:
                self.logger.error(f"Adaptive concurrency error: {e}")

    async def _adjust_concurrency(self, recorder: 'Recorder', disk_monitor: 'DiskSpaceMonitor'):
        active_downloads = len(recorder.sessions)
        has_space, free_mib = disk_monitor.check_space(recorder.global_config.save_path)

        target_limit = self._current_download_limit

        if not has_space:
            target_limit = max(self.config.max_concurrent_downloads_low, target_limit - 2)
            self.logger.warning(f"Low disk space ({free_mib} MiB), reducing download limit to {target_limit}")
        elif active_downloads >= self._current_download_limit * 0.9:
            target_limit = min(self.config.max_concurrent_downloads_high, target_limit + 1)
            self.logger.info(f"High utilization ({active_downloads}/{self._current_download_limit}), increasing limit to {target_limit}")
        elif active_downloads <= self._current_download_limit * 0.3:
            target_limit = max(self.config.max_concurrent_downloads_low, target_limit - 1)
            self.logger.info(f"Low utilization ({active_downloads}/{self._current_download_limit}), decreasing limit to {target_limit}")

        if target_limit != self._current_download_limit:
            await self._resize_download_semaphore(target_limit)
            self._current_download_limit = target_limit

    async def _resize_download_semaphore(self, new_limit: int):
        current_available = self._download_semaphore._value
        current_locked = self._download_semaphore.locked()

        if new_limit > self.config.max_concurrent_downloads:
            diff = new_limit - self.config.max_concurrent_downloads
            for _ in range(diff):
                self._download_semaphore.release()
        elif new_limit < self.config.max_concurrent_downloads:
    ...
    async def acquire_download_slot(self) -> bool:
        return self._download_semaphore.locked() or await self._try_acquire(self._download_semaphore)
    ...
    def get_stats(self) -> Dict[str, Any]:
        return {
            'download_limit': self._current_download_limit,
            'download_slots_used': self.config.max_concurrent_downloads - self._download_semaphore._value,
            'download_slots_available': self._download_semaphore._value,
            'fetch_slots_used': self.config.max_concurrent_fetches - self._fetch_semaphore._value,
            'fetch_slots_available': self._fetch_semaphore._value,
            'models_in_grace': len(self.offline_grace_tasks),
        }
...
# ========================
# Resource Monitor (Bandwidth + CPU)
# ========================

class ResourceMonitor:
    """Monitor system resources and enforce limits."""

    def __init__(self, bandwidth_limit_mbps: int = 0, cpu_limit_percent: int = 80):
        self.bandwidth_limit_bps = bandwidth_limit_mbps * 1_000_000 if bandwidth_limit_mbps > 0 else 0
        self.cpu_limit_percent = cpu_limit_percent
        self.logger = logging.getLogger("resource_monitor")
        self._total_bandwidth_bps = 0.0
        self._bandwidth_lock = asyncio.Lock()
        self._last_cpu_check = 0.0
        self._cpu_percent = 0.0

    async def check_bandwidth(self, current_bps: float) -> bool:
        """Check if adding current_bps would exceed bandwidth limit."""
        if self.bandwidth_limit_bps <= 0:
            return True
        async with self._bandwidth_lock:
            projected = self._total_bandwidth_bps + current_bps
            return projected <= self.bandwidth_limit_bps

    async def reserve_bandwidth(self, bps: float):
        async with self._bandwidth_lock:
            self._total_bandwidth_bps += bps

    async def release_bandwidth(self, bps: float):
        async with self._bandwidth_lock:
            self._total_bandwidth_bps = max(0, self._total_bandwidth_bps - bps)
    ...
    def is_cpu_overloaded(self) -> bool:
        return self._cpu_percent > self.cpu_limit_percent

    def get_stats(self) -> Dict[str, Any]:
        return {
            'bandwidth_limit_mbps': self.bandwidth_limit_bps / 1_000_000 if self.bandwidth_limit_bps > 0 else 0,
            'current_bandwidth_mbps': self._total_bandwidth_bps / 1_000_000,
            'cpu_limit_percent': self.cpu_limit_percent,
            'current_cpu_percent': round(self._cpu_percent, 1),
